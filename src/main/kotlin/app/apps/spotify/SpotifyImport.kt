...
object SpotifyImport: SpotifyApp(), Importer {
    override fun runImport() {
        ...
        try {
            ...
            val createdPlaylists = Playlist.createdPlaylists.filter { it.app.name == Apps.SPOTIFY.name }
    ...
    private fun getCurrentTracks(rawTracks: List<HashMap<String, *>>, playlistTitle: String): List<Track> {
        return rawTracks.map { rawTrack ->
            val artist = getArtist(rawTrack[ARTISTS] as List<HashMap<String, *>>)
            ...
            Track.createdTracks.filter { it.app.name == Apps.SPOTIFY.name }.firstOrNull { it.id == trackId }
                ?: Track(artist, trackName, trackId, isAvailable, Apps.SPOTIFY)
        }
    }

    private fun getArtist(artists: List<HashMap<String, *>>): Artist {
        return artists.let {
            val fullArtist = if (it.size == 1) {
                it.first()
            } else {
                it.maxByOrNull { currentArtist ->
                    val fullArtistURL = currentArtist[HREF] as String
                    val fullArtist = URLPlusToken(fullArtistURL, currentToken).getURLResponse()

                    fullArtist[POPULARITY] as Int
                }!!
            }

            val artistName = fullArtist[NAME].toString()
            val artistId = fullArtist[ID].toString()
